...
<input class="hidden" id ="Id" name="userId" type="text" value="" >

<div class="container-fluid bg-grey">
 	    <c:if test="${message != null}">
			<div class="alert alert-${message.type}" role="alert"><fmt:message key="${message.text}"  bundle="${bndmessages}"/></div>
		</c:if>
 	   <h2><fmt:message key="label.listofusers"  bundle="${bndmmenu}"/>
 	   </h2>
 	   
 	   <table class="table table-striped table-hover table-condensed " id="usertab" style="width:95%;">
            <thead>
            <tr>
                <th><h3><fmt:message key="tbl.userid" bundle="${bndmmenu}"/></h3></th>
                <th><h3><fmt:message key="tbl.utype" bundle="${bndmmenu}"/></h3></th>
                <th><h3><fmt:message key="tbl.userlogin" bundle="${bndmmenu}"/></h3></th>
<%--                 <th><h3><fmt:message key="tbl.userpass" bundle="${bndmmenu}"/></h3></th> --%>
            </tr>
            </thead>
            <tbody>
			<c:forEach var="usr" items="${userlist}" varStatus="count">
             <tr>
                <td class="shrink">${usr.userId}</td>
                <td align="left">
	                <c:forEach var="utype" items="${utypes}">
	                	<c:if test="${usr.uTypeId == utype.uTypeId}">
	                		${utype.uTypeName}
	                	</c:if>
	                </c:forEach>
                </td>
                
                <td align="left" id="row${usr.userId}">${usr.userLogin}</td>
<%--                 <td align="left"> <input  type="password"  value="${usr.userPass}" style="max-width:7em" readonly> --%>
                </td>
                <td>
                    <button type="button" class="btn btn-primary btn-xs" id="${usr.userId}"
                            onclick="doUpdate(event,'ShUserUpdate')"><fmt:message key="button.update"  bundle="${bndmmenu}"/>
                    </button>
                </td>
                <td>
                    <button  type="button" class="btn btn-warning btn-xs" id="${usr.userId}"
...
<div class="text-center">      
   <div class="btn-group"> 
      <button type="button" id="crtupd" class="btn1  btn-default btn-lg btn-success" 
   			onclick="setCommand3('ShUserCreate')">
				<fmt:message key="button.crtupd0" bundle="${bndmmenu}" />
	  </button>
      <button type="button" class="btn1   btn-default btn-lg btn-info" onclick="setCommand3('ShUserFilter')">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary modalDeleteBtn" name=""
                       onclick="doDelete('UserDelete')"><fmt:message key="button.delete"  bundle="${bndmmenu}"/>
                    </button>
                </div>
            </div>
        </div>
...
function showDelete(event) {
	...
	name = $('#usertab #row' + event.target.id).text();
	if (!latch){ 
